...
def check_prerequisites(username, model_type="svm"):
    ...
    if not has_trained_data(username):
        return (False, f"❌ Dữ liệu khuôn mặt cho {username} chưa được thu thập. Vui lòng liên hệ admin.", None)
    ...
    try:
        ...
        if not hasattr(recognizer, "predict_with_confidence"):
            return (False, "❌ Mô hình không hợp lệ: thiếu phương thức predict_with_confidence", None)
...
def process_frame_and_recognize(cap, recognizer, username, action, video_placeholder, video_file):
    """
    ...
    try:
        ...
        while cap.isOpened() and not recognized and attempt < max_attempts:
            ...
            if not ret:
                ...
                print(f"[LỖI] Không đọc được khung hình từ {'video' if video_file else 'webcam'}")
                break
            ...
            for x, y, w, h in faces:
                ...
                try:
                    ...
                    print(f"[NHẬN DIỆN]: name={name}, confidence={confidence:.4f}, username={username}")
                    ...
                    if label == username:
                        if confidence >= 0.5:
                            ...
                            if not is_allowed:
                                ...
                                success, msg = append_attendance_log(username, roi, "attendance", action)
                                result_message = f"✅ {msg}" if success else f"❌ {msg}"
                                recognized = True
                                break
                        else:
                            result_message = ("❌ Độ tin cậy thấp. Vui lòng check-in lại.")
                            print(f"Confidence = {confidence} dưới ngưỡng 0.5")
                            recognized = True
                            
                            display_message(result_message, is_success=False, placeholder=video_placeholder)
                            break
                    else:
                        result_message = "❌ Khuôn mặt không xác định (unknown)"
                        print(f"Bỏ qua khuôn mặt: label={label}, không khớp với username={username}")
                        recognized = True
                        break
        ...
        if not recognized and not result_message:
            result_message = ("❌ Không nhận diện được khuôn mặt sau nhiều lần thử. Vui lòng thử lại.")
...
def recognize_and_log(action="check-in", video_file=None):
    ...
    recognized, result_message = process_frame_and_recognize(cap, recognizer, username, action, st.empty(), video_file)
