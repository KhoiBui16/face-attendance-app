...
def main():
    ...
    input_source = st.radio(
        "Nguồn điểm danh:", ["Webcam", "Tải video"], horizontal=True
    )
    video_file = None
    ...
    with col1:
        if st.button("Check-in"):
            handle_check_in(
                input_source=input_source, video_file=video_file, username=username
            )
    ...
    if st.session_state.result_message:
        # Chỉ hiển thị nút "Thử lại" khi check-in và lỗi là "Khuôn mặt không xác định (unknown)"
        if st.session_state.result_message.startswith("✅"):
            ...
            )  # Hiển thị lỗi khác, không có nút "Thử lại"
